...
import { Building2, Calendar, GraduationCap, GripVertical, Pencil, Search, Trash2 } from "lucide-react";
...
const ICON_STYLES = [
  { bg: "bg-violet-100 text-violet-600 admin-dark:bg-violet-900/30 admin-dark:text-violet-300", icon: Building2 },
  { bg: "bg-teal-100 text-teal-600 admin-dark:bg-teal-900/30 admin-dark:text-teal-300", icon: GraduationCap },
  { bg: "bg-orange-100 text-orange-600 admin-dark:bg-orange-900/30 admin-dark:text-orange-300", icon: GraduationCap },
  { bg: "bg-blue-100 text-blue-600 admin-dark:bg-blue-900/30 admin-dark:text-blue-300", icon: Building2 },
];
...
export function EducationListCard({ items, loading, onEdit, onDelete }: EducationListCardProps) {
    ...
    <div className={cn(adminCardClass, "rounded-2xl p-5 sm:p-6")}>
      <div className="flex items-start gap-3 mb-6">
        ...
        <div>
          ...
          <p className={cn("text-sm", adminMutedClass)}>Manage your existing educational details.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p className={cn("text-center py-12 text-sm", adminMutedClass)}>
          {items.length === 0 ? "No education entries yet." : "No education matches your search."}
        </p>
      ) : (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-sm truncate">{item.degree}</p>
                    <p className={cn("text-sm truncate", adminMutedClass)}>{item.institution}</p>
                    {item.period && (
