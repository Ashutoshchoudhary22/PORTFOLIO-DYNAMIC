...
import { Briefcase, GripVertical, Pencil, Search, Trash2 } from "lucide-react";
...
export function ExperienceListCard({ items, loading, onEdit, onDelete }: ExperienceListCardProps) {
    ...
    <div className={cn(adminCardClass, "rounded-2xl p-5 sm:p-6")}>
      <div className="flex items-start gap-3 mb-6">
        ...
        <div>
          ...
          <p className={cn("text-sm", adminMutedClass)}>Manage your existing experiences.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p className={cn("text-center py-12 text-sm", adminMutedClass)}>
          {items.length === 0 ? "No experience entries yet." : "No experiences match your search."}
        </p>
      ) : (
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                {item.iconUrl ? (
                  ...
                  <div
                    className={`h-11 w-11 rounded-xl shrink-0 flex items-center justify-center text-sm font-bold text-violet-600 bg-violet-100 admin-dark:bg-violet-900/30 admin-dark:text-violet-300`}
                  >
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-sm truncate">{item.role}</p>
                  <p className={cn("text-sm truncate", adminMutedClass)}>{item.company}</p>
                  {item.period && (
                    <p className="text-xs text-slate-400 admin-dark:text-white/40 truncate">
                      {item.period}
                    </p>
                  )}
                </div>
